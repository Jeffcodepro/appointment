...
class SchedulesController < ApplicationController
  ...
  def history
    ...
    scope =
      case order_param
      when "recent"        then base.order(start_at: :desc)
      when "oldest"        then base.order(start_at: :asc)
      when "price_desc"    then base.left_joins(:service).order(Arel.sql("services.price_hour DESC NULLS LAST"))
      when "price_asc"     then base.left_joins(:service).order(Arel.sql("services.price_hour ASC  NULLS LAST"))
      when "duration_desc" then base.order(Arel.sql("COALESCE(EXTRACT(EPOCH FROM (end_at - start_at)),0) DESC"))
      when "duration_asc"  then base.order(Arel.sql("COALESCE(EXTRACT(EPOCH FROM (end_at - start_at)),0) ASC"))
      when "status_asc"
        cases = Schedule.statuses.map { |k, v| "WHEN #{v} THEN '#{k}'" }.join(" ")
        base.order(Arel.sql("CASE schedules.status #{cases} END ASC")).order(start_at: :desc)
      else
        base.order(start_at: :desc)
      end

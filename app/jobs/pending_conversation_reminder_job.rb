class PendingConversationReminderJob < ApplicationJob
  ...
  def perform
    Conversation.find_each do |conv|
      ...
      # só quem deve responder recebe
      if last_msg.user_id == conv.client_id
  ...
  def remind_side(conv, role, last_read_at, last_email_at)
    ...
    ConversationMailer.with(conversation: conv, recipient: user).pending_reminder.deliver_later
